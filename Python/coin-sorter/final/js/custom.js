...
function calibrate() {
	...
	cam_ip = $("#urlinput").val()
	 $.ajax({
	 	url: "src/CalibrateEdges.py?url="+cam_ip, 
...
//on clicing start button will call Start.py,on success to main_cnrlr() and arm_cnrlr() function
function start(){
	$("#start").hide();
	// $("#utility").hide();
	$("#loding").show();
...
function utility(count){
	...
	d = new Date();
	$("#loding").hide();
	$("#utility_img").attr("src" ,"config/util_img.png?"+d.getTime())
	$("#utility").show();
}
