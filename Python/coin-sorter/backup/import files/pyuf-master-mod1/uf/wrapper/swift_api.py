...
import re #custom package modification
...
class SwiftAPI():
    ...
    def reset(self):
        ...
        sleep(1) #Custom time sleep
        self.set_position(150, 0, 150, speed = 80, wait = True)
        sleep(1) #Custom time sleep
        self.set_pump(False)
        #custom changes to the package commented one line below
        #self.set_gripper(False)
        self.set_wrist(90)
        sleep(0.9) #Custom time sleep
    ...
    def set_position(self, x = None, y = None, z = None,
        ...
        #custom code for realising error in movement
        if(ret.startswith('ok') or ret.startswith('OK')):
            return True
        else:
            print('err, movement unavailable for command {}, last ret: {}'.format(cmd, ret))
            return False
        '''
    ...
    def set_pump(self, on, timeout = None):
        ...
        #custom changes to the below one line where if condition changed
        if (ret.startswith('ok') or ret.startswith('OK')):
            return True
